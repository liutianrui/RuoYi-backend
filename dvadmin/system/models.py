...
def media_jsonResult_name(instance, filename):
    h = instance.md5sum
    basename, ext = os.path.splitext(filename)
    return PurePosixPath("jsons", h[:1], h[1:2], h + ext.lower())
...
class FileList(CoreModel):
    ...
    # json_result = models.FileField(upload_to=media_jsonResult_name, null=True, blank=True, )
    # json_result_url = models.CharField(max_length=255, blank=True, verbose_name="json分析结果文件地址", help_text="json分析结果文件地址")
    ...
    dict = models.CharField(max_length=2000, null=True, blank=True, verbose_name="分析结果", help_text="分析结果")
    ...
    def save(self, *args, **kwargs):
        ...
        #     json_result = media_jsonResult_name(self, self.name + 'classifyResults.json')
        #     self.json_result_url = f'media/{json_result}'
